...
list.of.packages <- c("hydroTSM", "lubridate",'mxnet','openxlsx','reshape2','plyr','neuralnet',"stringr","hydroGOF","pracma","RSNNS","doParallel") #list pacotes
...
train.x = data.matrix(train_[,2:6])
train.y = train_[,1]
test.x = data.matrix(test_[,2:6])
test.y = test_[,1]
...
data_ <- mx.symbol.Variable("data")
# A fully connected hidden layer
# data: input source
# num_hidden: number of neurons in this hidden layer
fc1 <- mx.symbol.FullyConnected(data_, num_hidden=1)

# Use linear regression for the output layer
lro <- mx.symbol.LogisticRegressionOutput(fc1)

mx.set.seed(0)
model <- mx.model.FeedForward.create(lro, X=train.x, y=train.y,
                                     ctx=mx.cpu(),     num.round=50, array.batch.size=20,
                                     learning.rate=5e-4, momentum=0.9,  eval.metric=mx.metric.rmse)
...
pr.nn_ <- denormalizeData(pr.nn,getNormParameters(test.x)) #só normalizando
